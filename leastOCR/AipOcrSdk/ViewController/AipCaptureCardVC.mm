...
@property (weak, nonatomic) IBOutlet UIImageView *leftEmoployImageView;
@end
...
-(void)config{
    ...
    if (self.CaptureCardscale == 0) {
        
        self.cutImageView.scale = 1.0;
        
    }else{
        
        self.cutImageView.scale = self.CaptureCardscale;
    }
...
-(void)imageRequestAction{
    ...
    switch (self.cardType) {
        ...
        case CardTypeBankCard: {  //  银行卡
            ...
            break;
        case CardTypeLocalIdCardFont:{//  扫描识别身份证正面
            
            [self detectLocalIdcard:NO];
        }
            break;
        case CardTypeLocalIdCardBack:{//  扫描识别身份证背面
    ...
    switch (self.cardType) {
        ...
        case CardTypeIdCardFont:
        {
            ...
            if (!self.iSEmployedCard) {//  从业资格证
                ...
                self.emblemImageView.hidden = YES;
                self.leftEmoployImageView.hidden = NO;
                self.tipsLabel.text = MyLocal(@"对齐从业资格证正面");
            }
